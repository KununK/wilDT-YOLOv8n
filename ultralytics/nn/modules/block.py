...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, DCNV3_YOLO
...
class AVG_SPPFCSPC(nn.Module):
    
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(AVG_SPPFCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        x4 = self.n(x1)
        x5 = self.n(x4)
        y1 = self.cv6(self.cv5(torch.cat((x1,x2,x3,x4,x5, self.m(x3), self.n(x5)),1)))
        y2 = self.cv2(x)
        return self.cv7(torch.cat((y1, y2), dim=1))

# 有一点成效
class AVG_SPPFCSPC1(nn.Module):
    
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(AVG_SPPFCSPC1, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
...
#         c_ = int(c2 * e)  # hidden channels
#         self.cv1 = Conv(c1, c_, k[0], 1)
#         self.cv2 = DCNV4_YOLO(c_, c2, k[1], 1, g=g)
#         self.add = shortcut and c1 == c2
